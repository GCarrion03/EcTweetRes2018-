...
var Helper = {
              ...
              plotOptions: {
            	  series: {
                      ...
                      point: {
                          events: {
                              click: function () {
                            	  $("#query").val(this.category);
                            	  $("#btn-search").click();
                            	  //window.open('https://twitter.com/statuses/'+this.category)
                            	  //alert('Text : ' + tweets[this.index].text + ' Retweets: '+ this.y);
    ...
    getTopUsersBarChartData: function(response,title,yAxisLabel,symbol){
...
//                                	  $("#query").val(this.category);
//                                	  $("#btn-search").click();
                                	  window.open('https://twitter.com/'+this.category);
                                	  //alert('Text : ' + tweets[this.index].text + ' Retweets: '+ this.y);
    ...
    getTop5RetweetsBarChartData: function(response,title,yAxisLabel,symbol){
    ...
    getTopTweetsBarChartData: function(response,title,yAxisLabel,symbol){
        ...
        if (response !== undefined && response !== null){
          ...
          $.each(response.hits.hits,function(k,value){
        	  var i=0;
        	  $.each(tweets,function(k,value1){
        		  if (value1.id==value._source.retweeted_status.id){
        		  	i++;
        	  	  }
        	  });
        	  if (i==0 && tweets.length<=10) {
	            trends.push(symbol+value._source.retweeted_status.text);
	            counts.push(value._source.retweeted_status.retweet_count);
	            tweets.push(value._source.retweeted_status);
        	  }
            ...
            var dataChart =  {
                  ...
                  exporting: {enabled: true }, 
                  title:     {text: title }, 
                  xAxis:     {categories: trends,data: tweets }, 
                  yAxis:     {title: {text: yAxisLabel } }, 
                  plotOptions: {
                	  series: {
                          cursor: 'pointer',
                          point: {
                              events: {
                                  click: function () {
                                	  window.open('https://twitter.com/statuses/'+tweets[this.index].id_str)
                                	  //alert('Text : ' + tweets[this.index].text + ' Retweets: '+ this.y);
