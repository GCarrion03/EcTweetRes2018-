...
#                writeLog(str(err)+str(tweet["place"])+"Error determining the tweet origin, city:"+str(tweet["place"]["name"]))
            self.tweet_count += 1
	    ...
	    try:
	       emailer.sendEmail(message=str(notice_msg))
	    	    ...
	    	    sys.stdout.write("Error sending mail proceeding with harvesting without notification")
        except:
...
try:
    ...
    writeLog(notice_msg)
    sys.stdout.write("using existing database")
    try:
       emailer.sendEmail(message=str(notice_msg))
    except Exception, err:
    	    sys.stdout.write("Error sending mail proceeding with harvesting without notification")
try:
    writeLog("Testing parameters ")
    writeLog("Locations: "+str(settings.locations))
    if settings.locations != []:
       writeLog("Locations: "+str(settings.locations))
       twitterStream.filter(locations=settings.locations)
    elif settings.tracks != [] and settings.follows != []:
       writeLog("Tracks: "+str(settings.tracks)+" Follows: "+str(settings.follows))
       twitterStream.filter(follow=settings.follows,track=settings.tracks)
except Exception, err:
